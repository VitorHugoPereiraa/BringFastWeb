...
const SingleInputAlert: React.FC<Props> = (props: Props) => {
    ...
    async function handleSubmit(){
     ...
     let placesCreatedByUser = data.docs.map(item => item.data())
      let placesWithSameId = placesCreatedByUser.filter(place => place.id == name)
     if(placesWithSameId.length) return window.alert("Você já possui uma mesa cadastrada com esse identificador!")
